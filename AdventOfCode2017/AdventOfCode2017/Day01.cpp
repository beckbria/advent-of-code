...
void Day1ATests()
{
    ...
    for (auto &t : testCases) {
        int const count = sumRepeatingDigits(t.input);
        if (count != t.answer) {
...
void Day1BTests()
{
    const struct {
        ...
        {"12131415", 4} };
    for (auto &t : testCases) {
        int const count = sumHalfwayAroundDigits(t.input);
        if (count != t.answer) {
...
void Day1()
{
    ...
    std::cout << sumRepeatingDigits(source) << std::endl;
    std::cout << sumHalfwayAroundDigits(source) << std::endl << std::endl;
}