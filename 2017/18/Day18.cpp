...
namespace Day18
{
    ...
    int64_t RecoverFrequency(const std::vector<std::string> &input)
    {
        ...
        program.SetSndHandler([&mostRecentFrequency](int64_t frequency)
                              { mostRecentFrequency = frequency; });
        program.SetRcvHandler([&recovered](int64_t condition)
                              { recovered = (condition != 0); });
    ...
    int64_t ParallelExecution(const std::vector<std::string> &input)
    {
        ...
        while ((!program0.Blocked() || !program1.Blocked()) && // Stop if we get to deadlock
               (!program0.Complete() || !program1.Complete()))
        { // Stop if both programs have finished
...
void Day18Tests()
{
    ...
    if (result != expected)
        std::cerr << "Test 18A Error: Got " << result << ", Expected " << expected << std::endl;
    ...
    if (resultB != expectedB)
        std::cerr << "Test 18B Error: Got " << resultB << ", Expected " << expectedB << std::endl;
}
...
void Day18Problems()
{
    ...
    auto input = Helpers::ReadFileLines("2017/18/input_day18.txt");
