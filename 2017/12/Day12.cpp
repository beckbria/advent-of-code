...
namespace Day12
{
    ...
    std::map<int, std::shared_ptr<Node>> BuildGraph(const std::vector<std::string> &input)
    {
        ...
        for (auto &line : input)
        {
            ...
            if (nodes.count(originValue) == 0)
                nodes[originValue] = std::make_shared<Node>(originValue);
            ...
            for (size_t i = 2; i < tokens.size(); ++i)
            {
                ...
                if (nodes.count(neighborValue) == 0)
                    nodes[neighborValue] = std::make_shared<Node>(neighborValue);
    ...
    unsigned int ConnectedNodeCount(std::shared_ptr<Node> root)
    {
...
void Day12Tests()
{
    ...
    if (count != 6)
        std::cerr << "Test 12A Error: Got " << count << ", Expected 6" << std::endl;
    int totalGroups = TotalGroups(nodes);
    if (totalGroups != 2)
        std::cerr << "Test 12B Error: Got " << totalGroups << ", Expected 2" << std::endl;
}
...
void Day12Problems()
{
    ...
    auto input = Helpers::ReadFileLines("2017/12/input_day12.txt");
    auto nodes = Day12::BuildGraph(input);
