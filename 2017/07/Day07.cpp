        ...
        int TowerWeight()
        {
            ...
            if (m_towerWeight < 0)
            {
                ...
                for (auto &child : children)
                {
                    ...
                    if ((child->TowerWeight() != childWeight) || (!child->BalancedChildren()))
                        m_balancedChildren = false;
                }
                m_towerWeight = tower;
            }
    ...
    int BalanceTree(std::shared_ptr<Node> root, int desiredShift)
    {
        ...
        for (auto &child : root->children)
        {
            ...
            if (!child->BalancedChildren())
            {
                ...
                if (desiredShift != 0)
                    return BalanceTree(child, desiredShift);
        ...
        if (childWithUnequalWeight)
        {
            ...
            else
            {
                ...
                if (desiredShift != 0)
                {
                    for (auto &child : root->children)
                    {
                        if (child->TowerWeight() + desiredShift == child0Weight)
                            return child->GetWeight() + desiredShift;
                        if (child->TowerWeight() == child0Weight + desiredShift)
                            return root->children[0]->GetWeight() + desiredShift;
                    }
                }
                else
...
void Day7Tests()
{
    ...
    if (root->name != "tknk")
        std::cout << "Test 7A Failure: Got " << root->name << ", Expected tknk" << std::endl;
    auto newWeight = Day7::BalanceTree(root, 0);
    if (newWeight != 60)
        std::cout << "Test 7B Failure: Got " << newWeight << ", Expected 60" << std::endl;
}
...
void Day7Problems()
{
    ...
    const auto input = Helpers::ReadFileLines("2017/07/input_day7.txt");
    auto root = Day7::BuildTree(input);
