...
void Day21Tests()
{
    ...
    if (pixels != expectedPixels)
        std::cerr << "Test 21A Error: Got " << pixels << ", Expected " << expectedPixels << std::endl;
}
...
void Day21Problems()
{
    ...
    const auto input = Helpers::ReadFileLines("2017/21/input_day21.txt");
    ...
    for (auto i = grid.Iteration(); i < 18; ++i)
        grid.Tick();
    const auto pixelsAfter18 = grid.SetBits();
    const auto end = std::chrono::steady_clock::now();
    std::cout << pixelsAfter5 << std::endl
              << pixelsAfter18 << std::endl;
