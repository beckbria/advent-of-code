...
namespace Day20
{
    ...
    std::vector<Particle> ReadParticles(const std::vector<std::string> &input)
    {
        ...
        {
            if (!input[i].empty())
                particles.emplace_back(input[i], i);
        }
        return particles;
    }
    ...
    bool WillNeverCollide(const std::vector<Particle> &particles)
    {
        ...
        for (const auto &p : particles)
        {
            ...
            if ((currentDistance < previousDistance) ||
                ...
                !SameDirection(p.Velocity(), p.Position()))
                return false;
...
void Day20Tests()
{
    ...
    if (closest != expectedClosest)
        std::cerr << "Test 20A Error: Got " << closest << ", Expected " << expectedClosest << std::endl;
    ...
    if (remaining != expectedRemaining)
        std::cerr << "Test 20B Error: Got " << remaining << ", Expected " << expectedRemaining << std::endl;
}
...
void Day20Problems()
{
    ...
    const auto input = Helpers::ReadFileLines("2017/20/input_day20.txt");
    const auto particles = Day20::ReadParticles(input);
    const auto closest = Day20::ClosestToOriginInLongTerm(particles);
