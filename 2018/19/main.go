...
func ReadComputer(input []string) Computer {
	...
	return Computer{ir: ir, ip: 0, reg: reg, inst: inst}
}
...
func main() {
	...
	comp.run()
	fmt.Println(comp.reg[0])*/
	// Instead, reverse-engineer the assembly, find the
	// hideously inefficient divisor checks, and.....
	sum := int64(0)
	target := 10551358
	for i := 1; i <= target; i++ {
		if target%i == 0 {
